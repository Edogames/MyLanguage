...
static const char* c_type_name(const char* type) {
    ...
    if (strcmp(type, "Window") == 0) return "Window";
    return type;
}
...
static char* transform_window_functions(const char* expr) {
    StrBuf sb;
    sb_init(&sb);
    for (int i = 0; expr[i]; i++) {
        if (!inside_string_at(expr, i) && strncmp(expr + i, "Window(", 7) == 0 &&
            (i == 0 || !is_ident_char(expr[i - 1]))) {
            sb_append(&sb, "mlg_window_create(");
            i += 6;
        } else if (!inside_string_at(expr, i) && strncmp(expr + i, ".to_html()", 10) == 0) {
            /* This is a bit tricky, we need to wrap the variable.
               For now, we only handle simple variable.to_html() */
            char* start = (char*)expr + i - 1;
            while (start >= expr && is_ident_char(*start)) start--;
            start++;
            int var_len = (int)(expr + i - start);
            char var[128] = {0};
            snprintf(var, sizeof(var), "%.*s", var_len, start);
            
            /* Remove the variable from the buffer and replace with function call */
            sb.len -= (size_t)var_len;
            sb_appendf(&sb, "mlg_window_to_html(%s)", var);
            i += 9;
        } else {
            sb_append_char(&sb, expr[i]);
        }
...
static char* transform_expr(Codegen* cg, const char* expr) {
    ...
    char* s2w = transform_window_functions(s2);
    char* s3 = transform_methods(cg, s2w);
    char* s4 = transform_interpolation(cg, s3);
    ...
    free(s1); free(s2); free(s2w); free(s3); free(s4); free(s5); free(s6); free(s7);
    return s8;
}
...
static void emit_runtime(StrBuf* out) {
    ...
    sb_append(out,
	        ...
	        "typedef struct MlgWindow { int width; int height; string title; string content; } MlgWindow; typedef MlgWindow* Window;\n"
    ...
    sb_append(out,
	    ...
	    /* ── Windowing ── */
	    sb_append(out,
	        "Window mlg_window_create(int width,int height,string title,string content){\n"
	        "    Window w=malloc(sizeof(MlgWindow));\n"
	        "    w->width=width;w->height=height;\n"
	        "    w->title=string_dup(title);w->content=string_dup(content);\n"
	        "    return w;}\n"
	        "string mlg_window_to_html(Window w){\n"
	        "    if(!w)return string_dup(\"\");\n"
	        "    return mlg_format(\"<html><head><title>%s</title></head><body>\"\n"
	        "                      \"<div style='width:%dpx;height:%dpx;border:1px solid black;'>\"\n"
	        "                      \"<h1>%s</h1><p>%s</p></div></body></html>\",\n"
	        "                      w->title,w->width,w->height,w->title,w->content);}\n");
}
