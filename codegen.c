...
typedef enum {
    TARGET_NATIVE, TARGET_WIN, TARGET_LIN, TARGET_ANDROID, TARGET_IOS, TARGET_MAC
} BuildTarget;
...
typedef struct { char paths[128][512]; int count; } ImportSet;
...
static AstNode* parse_file(const char* path) {
    ...
    if (!source) { fprintf(stderr, "Open failed: %s\n", path); return NULL; }
    TokenArray tokens;
    if (!lex_source(source, path, &tokens)) { free(source); return NULL; }
...
static void prepend_imported_functions(AstNode* program, AstNode** funcs, int count) {
    ...
    NodeArray next; nodes_init(&next);
...
static int parse_options(int argc, char** argv, CompileOptions* options) {
    options->input = NULL; options->output = NULL;
    options->target = TARGET_NATIVE; options->backend_c = 0; options->run_bundle = 0;
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i],"--run")==0) { options->run_bundle=1; }
        else if (strcmp(argv[i],"--backend")==0) {
            if (i+1>=argc) { fprintf(stderr,"--backend requires a value: vm or c\n"); return 0; }
            const char* b=argv[++i];
            if (strcmp(b,"c")==0) options->backend_c=1;
            else if (strcmp(b,"vm")==0) options->backend_c=0;
            else { fprintf(stderr,"Unknown backend '%s'.\n",b); return 0; }
        } else if (strcmp(argv[i],"--target")==0) {
            if (i+1>=argc) { fprintf(stderr,"--target requires a value\n"); return 0; }
            options->target=parse_target(argv[++i]);
        } else if (!options->input) { options->input=argv[i]; }
...
static void final_output_name(const CompileOptions* options, char* out, size_t out_size) {
    ...
    /* C backend: native binary — no ext on Linux/Mac, .exe on Windows.
       VM backend: always .mlgb portable bundle. */
    const char* ext;
    if (options->backend_c) {
        ext = (target == TARGET_WIN) ? ".exe" : "";
    } else {
        ext = ".mlgb";
    }
    ...
    if (ext[0]) {
        size_t len = strlen(out), ext_len = strlen(ext);
        if (len < ext_len || strcmp(out+len-ext_len, ext) != 0)
...
static int build_command(BuildTarget target, const char* output, char* command, size_t command_size) {
    ...
    snprintf(command, command_size, "%s _temp_.c -o \"%s\" -Wall -Wextra%s",
             compiler, output, target==TARGET_WIN ? " -lws2_32" : " -lpthread");
    return 1;
}

static void print_target_note(BuildTarget target) {
    if (target==TARGET_ANDROID) printf("Target android: requires NDK clang in PATH.\n");
    else if (target==TARGET_IOS) printf("Target ios: requires Xcode command line tools.\n");
    else if (target==TARGET_MAC) printf("Target mac: requires clang + macOS SDK.\n");
    else if (target==TARGET_LIN && host_target()!=TARGET_LIN) printf("Target lin: run on Linux or use a cross-compiler.\n");
    else if (target==TARGET_WIN && host_target()!=TARGET_WIN) printf("Target win: requires x86_64-w64-mingw32-gcc in PATH.\n");
}

int mylang_main(int argc, char** argv) {
    CompileOptions options;
    if (argc < 2 || !parse_options(argc, argv, &options)) {
        printf("Usage: %s [--run] <file.mlg|file.mlgb> [output_name] [--backend vm|c] [--target win|lin|android|ios|mac]\n", argv[0]);
        return 1;
    }

    if (options.run_bundle) return vm_run_bundle(options.input);

    char* source = mlg_read_file(options.input);
    if (!source) { fprintf(stderr,"Open failed: %s\n",options.input); return 1; }
    ...
    if (!build_command(target, output, command, sizeof(command))) {
        fprintf(stderr,"No configured compiler for target '%s' on this host.\n", target_name(target));
        free(c_source); free(source); symbols_free(&symbols); return 1;
    }
    int status = system(command);
    free(c_source); free(source); symbols_free(&symbols);
    return status == 0 ? 0 : 1;
}
